  ...
  <header>
    <div class="left-box">
      ...
      <div class="menu-icon"
        ...
        <i v-if="isCollapse"
           class="iconfont icon-caidanzhankai"></i>
        <i v-else
           class="iconfont icon-caidanshouqi"></i>
      </div>
      <Breadcrumb />
    </div>
    <div class="right-box">
...
export default defineComponent({
  ...
  setup () {
    ...
    // actions.setGlobalState(store.state.user)
    actions.onGlobalStateChange((cur) => {
      store.commit('user/infoChange', cur)
    })

    const changeEmail = () => {
      actions.setGlobalState({ email: 123, username: 'zzzz' })
    }
    return {
      isCollapse,
      layer,
