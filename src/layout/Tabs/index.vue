<template>
  <div class="tabs">
    <el-scrollbar class="scroll-container tags-view-container"
                  ref="scrollbarDom">
      <Item v-for="menu in store.state.keepAlive.tabsList"
            :key="menu.meta.title"
            :menu="menu"
    ...
    <div class="handle">
      <el-dropdown placement="bottom">
        ...
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item :disabled="currentDisabled"
                              @click="closeCurrentRoute">关闭当前页面</el-dropdown-item>
            <el-dropdown-item :disabled="store.state.keepAlive.tabsList.length < 3"
                              @click="closeOtherRoute">关闭其他页面</el-dropdown-item>
            <el-dropdown-item :disabled="store.state.keepAlive.tabsList.length <= 1"
                              @click="closeAllRoute">关闭所有页面</el-dropdown-item>
          </el-dropdown-menu>
        </template>
      </el-dropdown>
    </div>
...
export default defineComponent({
  ...
  setup () {
    ...
    function closeOtherRoute () {
      ...
      store.commit('keepAlive/SET_TABS_LIST', menuList.value)
      if (route.path !== defaultMenu.path) {
        // eslint-disable-next-line no-use-before-define
        addMenu(route)
      }
    ...
    const delMenu = (menu) => {
      ...
      const microObj = {}
      Object.keys(store.state.keepAlive.microList).forEach((k) => {
        if (menu.path.startsWith(k) && store.state.keepAlive.tabsList.some((v) => v.path !== menu.path)) {
          store.state.keepAlive.microList[k].unmount()
        } else {
          microObj[k] = store.state.keepAlive.microList[k]
        }
      })
      store.commit('keepAlive/SET_MICRO_LIST', microObj)
    ...
    function setPosition () {
      ...
      //   const domBox = {
      //     scrollbar: scrollbarDom.value.scrollbar$ && scrollbarDom.value.scrollbar$.querySelector('.el-scrollbar__wrap'),
    ...
    store.subscribe((mutation) => {
      if (mutation.type === 'menuList/insertMenu') {
        menuList.value = store.state.keepAlive.tabsList
      }
    })
